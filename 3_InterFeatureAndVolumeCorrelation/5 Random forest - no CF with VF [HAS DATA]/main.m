...
parfor dBSRepIndex = 1:dNumBSReps+1
	...
	% Get correlation on training set
	dVolumeFeatureIndex = find(oRadiomicTrainingSet.GetFeatureNames() == "F020001");
	m2dFeatures = oRadiomicTrainingSet.GetFeatures();
	...
	vbHighlyVolumeCorrelatedFeatures = ...
        ...
        (vdPValuePerRadiomicFeatureForVolume < 0.05)); % this will remove volume feature too
		
	% Remove volume correlated feature (incl. volume)
	oRadiomicTrainingSet = oRadiomicTrainingSet(:,~vbHighlyVolumeCorrelatedFeatures);
	oRadiomicTestingSet = oRadiomicTestingSet(:,~vbHighlyVolumeCorrelatedFeatures);
