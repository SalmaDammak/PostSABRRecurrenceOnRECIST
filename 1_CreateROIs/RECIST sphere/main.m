...
chSphereFolderPath = [chBaseFolderPath,'\Segmentations - 5 - RECIST-sized sphere'];
...
for iOutcome = 1:length(stOutcomes)
    ...
    for iPatient = 1:length(stPatients)
        ...
        [m2dPoint1, m2dPoint2, m2dMidpoint, dLength] = QueryRECISTData(tRECISTComplete, chRECISTImageName, c1chCTName{1});
        
        if size(m2dPoint2, 1) > 1
            error("Too many RECIST matches found!")
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % GET RECIST diameter sphere
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        dRadius_mm = dLength/2;
        ...
        vsRECISTSizedSpherePaths = [vsRECISTSizedSpherePaths; string(strrep(chSphereRawFilePath, '.raw', '.mhd'))];
    end
end

save([Experiment.GetResultsDirectory(), '\Paths.mat'], "vsImagePaths", "vsRECISTSizedSpherePaths", "vsCylinderPaths");